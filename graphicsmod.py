...
#from graphics.mesh import simple_quad_mesh, simple_poly_mesh
...
# currently hoovering.                                     #
    ...
    def sendRay(self, mouse_pos):
        ...
        objs = []
        i = 0
        for rm in self.render_manager:
            ray_pick = coin.SoRayPickAction(rm.getViewportRegion())
            ray_pick.setPoint(coin.SbVec2s(*mouse_pos))
            ray_pick.setRadius(10)
            ray_pick.setPickAll(True)
            ray_pick.apply(rm.getSceneGraph())
            picked_point = ray_pick.getPickedPointList()
            obj = self.objByID(picked_point)
            #print("RM : %d -> OBJ : %s"%(i, obj))
            objs.append(obj)
            i += 1
        #print("objs : %s"%(objs))
        return(objs[0]) #picked_point)
...
#------------------------INTERACTION------------------------#
    ...
    def cursor_pos(self, event):
        ...
        pts = []
        for rm in self.render_manager:
            pts.append(getPointOnScreen(rm, pos))
